...
t = T / B  # T in units of binding energy
...
U_bebvm = np.linspace(0, 8, num=8,
                      endpoint=True)  # Bound electron kinetic energy in eV of the BEBVM approximation (currently for testing)
tbar = T / (m_e * c ** 2)
bbar = B / (m_e * c ** 2)
...
f_2 = - phi / (t + 1) * (1 + 2 * tbar) / (1 + tbar / 2) ** 2
f_3 = np.log(beta_t * (1 + tbar) ** 2) - beta_t - np.log(2 * bbar)


def total_cross_section_bebvm(w, t=t, tbar=tbar, bbar=bbar, f_2=f_2, f_3=f_3):
    data_set_len = len(U_bebvm)
    total_cross_sec = np.zeros(data_set_len)
    for i in range(data_set_len):
        f_1 = 4 * np.pi * alpha ** 4 * a_0 ** 2 * N / (2 * bbar * (beta_t + (beta_b + beta_u[i])) / 2)
        u = U_bebvm[i] / B
        total_cross_sec[i] = np.sum(f_1 * (
                    f_2 * (np.log((w + 1) / (t - w) * t)) - 1 / (w + 1) + 1 + 1 / (t - w) - 1 / t + bbar ** 2 * w / (
                        1 + tbar / 2) ** 2 + f_3 / 2 * (1 / (t - w) ** 2 - 1 / t ** 2 - 1 / (w + 1) ** 2 + 1)))
    return 1.e6 * total_cross_sec


for i in range(len(U_bebvm)):
    w_max = ((T + U_bebvm[i]) / B - 1) / 2  # Devided by 2 since the outgoing electrons are indistinguishable
...
def differential_cross_section_subshells_bed(w, t=t, U=U_bed, B=B, f_1=f_1_bed, ai_below=ai_below_2s_ion_threshold,
                                             ai_above=ai_above_2s_ion_threshold):
    N_shells = len(w)  # Number of shells (equivalent to number of integrations in the end)
    u = U / B
    osc_str = np.zeros(N_shells)
    for i in range(N_shells):
        ai = ai_below if (w[i] + u[i] - 1.) * B[
            i] < 48.47 else ai_above  # different coefficients for different engergy regimes, W+U-B heeby corresponds to the

        # if (w[i] + u[i] - 1.) * B[i] < 48.47:
        # 	ai = ai_below
        # else:
        # 	ai = ai_above
        ...
        diff_cross_sec_subshells = f_1 / (t + u + 1.) * (
                    (Ni / N - 2.) / (t + 1.) * (1. / (w + 1.) + 1. / (t - w)) + (2. - Ni / N) * (
                        1. / (t - w) ** 2 + 1. / (w + 1.) ** 2.) + np.log(t) / N * np.dot(1. / (w + 1.),
...
def total_cross_section_bed(T=T, B=B, U=U_bed):
    w_max = ((T + U) / B - 1) / 2
